...
namespace Game
{
    public class UIManager : MonoBehaviour
    {
        ...
        {
            if (ResourceManager.GetCompletedLevel(LevelManager.Instance.GameMode) < LevelManager.Instance.Level.no)
            {
                _levelCompletePanel.coin.SetActive(true);
                GameManager.GOLD_COIN += 50;   
                _levelCompletePanel.coinText.text = "X "+ _levelCompletePanel.rewardCoin;
            }
            else
            {
                _levelCompletePanel.coin.SetActive(false);
                _levelCompletePanel.coinText.text = "Good Job!!";
            }
