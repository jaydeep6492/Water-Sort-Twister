...
public class Holder : MonoBehaviour
{
    ...
    [SerializeField] private float _offsetLiquidY;
    [SerializeField] private IEnumerable<LiquidData> liquidDatas;
    ...
    public void AddLiquid(int groupId, float value = 0)
    {
        ...
        offset = _liquids.Aggregate(offset, (current, liquides) => current + new Vector2(0, _offsetLiquidY) * liquides.Value);
        liquid.transform.position = topPoint + offset;
        liquid.Value = value;
        _liquids.Add(liquid);
    }
    ...
    public void RemoveAtLast(float value)
    {
        var last = Liquids.Last();
        if (last.IsBottomLiquid && last.Value - value > 0)
        {
            last.Value -= value;
        }
        else
        {
            ...
            _liquids.RemoveAt(_liquids.Count-1);
        }
    }
    
    public void SwapLiquid()
    {
        var list = new List<LiquidData>();
        for (var i = 0; i < _liquids.Count; i++)
        {
            for (var j = i+1; j < _liquids.Count; j++)
            {
                if (_liquids[i].GroupId == _liquids[j].GroupId)
                {
                    list.Add(new LiquidData()
                    {
                        groupId = _liquids[i].GroupId,
                        value = _liquids[i].Value + _liquids[j].Value 
                    });
                }
            }
        }
        ...
        {
            foreach (var itemData in list.Where(itemData => itemData.groupId ==item.GroupId))
            {
                goto forContinue;
            }
            list.Add(new LiquidData()
